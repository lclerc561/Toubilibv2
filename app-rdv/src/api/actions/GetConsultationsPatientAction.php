...
class GetConsultationsPatientAction
{
    ...
    public function __construct(
        ServiceRDVInterface $serviceRDV,
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        // OPTIMISATION : Récupérer tous les praticiens UNIQUES en un seul passage
        $praticienIds = array_unique(array_map(fn($c) => $c->praticienId, $consultations));
        
        // Cache des praticiens : un seul appel HTTP par praticien unique
        $praticiensCache = [];
        foreach ($praticienIds as $praticienId) {
            $praticien = $this->praticienAdapter->getPraticien($praticienId);
            $praticiensCache[$praticienId] = $praticien;
        }

        // Construire la réponse avec les praticiens depuis le cache
        $data = [];
        foreach ($consultations as $consultation) {
            $praticien = $praticiensCache[$consultation->praticienId] ?? null;
